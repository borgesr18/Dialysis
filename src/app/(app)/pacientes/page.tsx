  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Link href="/pacientes/new" className="btn"><i className="fa-solid fa-plus mr-2" /> Novo paciente</Link>
      </div>
      ...
      <form className="card grid md:grid-cols-4 gap-3">
        ...
        <button className="btn md:col-start-4" type="submit"><i className="fa-solid fa-magnifying-glass mr-2" /> Filtrar</button>
      </form>
        ...
        {(pacientes ?? []).map((p) => (
          <div key={p.id} className="card flex items-center justify-between">
            <div>
              ...
              <div className="text-sm text-neutral-600">REG {p.registro} • {p.cidade_nome ?? '—'}</div>
              {p.alerta_texto && (
            ...
            <div className="flex items-center gap-2">
              <Link className="px-3 py-1.5 rounded-md border hover:bg-gray-50 text-sm" href={`/pacientes/${p.id}`}>
              ...
              <form action={async () => { 'use server'; await toggleAtivo(p.id, !p.ativo); }}>
                <button className="px-3 py-1.5 rounded-md border hover:bg-gray-50 text-sm" type="submit">
                  {p.ativo ? (<><i className="fa-solid fa-user-slash mr-1" /> Inativar</>) : (<><i className="fa-solid fa-user-check mr-1" /> Ativar</>)}
                </button>
              </form>
              ...
              <form
                action={async () => {
                  'use server';
                  // proteção simples
                  // (para confirmação no client, use um confirm() num botão client-side se quiser)
                  await removePaciente(p.id);
                }}
              >
                <button className="px-3 py-1.5 rounded-md border hover:bg-red-50 text-sm text-red-600" type="submit">
