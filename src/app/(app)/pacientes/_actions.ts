...
function parsePacienteForm(fd: FormData) {
  try {
    const rawData = {
      ...
      data_nascimento: String(fd.get('data_nascimento') ?? '').trim(),
      sexo: String(fd.get('sexo') ?? '').trim() as 'M' | 'F',
      telefone: fd.get('telefone') ? String(fd.get('telefone')).trim() : undefined,
      endereco: fd.get('endereco') ? String(fd.get('endereco')).trim() : undefined,
      convenio: fd.get('convenio') ? String(fd.get('convenio')).trim() : undefined,
      numero_convenio: fd.get('numero_convenio') ? String(fd.get('numero_convenio')).trim() : undefined,
